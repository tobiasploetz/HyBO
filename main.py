...
def HyBO(
    ...
    **kwargs,
    ...
    if objective is not None:
        ...
        log_order_variances = torch.zeros(
            (objective.num_discrete + objective.num_continuous)
        )
    ...
    for i in range(n_eval):
        ...
        eval_outputs = torch.cat(
            [eval_outputs, objective.evaluate(eval_inputs[-1]).view(1, 1)]
        )
        assert not torch.isnan(eval_outputs).any()
    ...
    for i in range(1):
        if objective_ == "coco":
            ...
            kwag_["objective"] = MixedIntegerCOCO(
                random_seed_, problem_id=kwag_["problem_id"]
            )
        elif objective_ == "weld_design":
            random_seed_ = sorted(generate_random_seed_coco())[i]
            kwag_["objective"] = Weld_Design(
                random_seed_, problem_id=kwag_["problem_id"]
            )
        elif objective_ == "speed_reducer":
            random_seed_ = sorted(generate_random_seed_coco())[i]
            kwag_["objective"] = SpeedReducer(
                random_seed_, problem_id=kwag_["problem_id"]
            )
        elif objective_ == "pressure_vessel":
            random_seed_ = sorted(generate_random_seed_coco())[i]
            kwag_["objective"] = Pressure_Vessel_Design(
                random_seed_, problem_id=kwag_["problem_id"]
            )
        ...
        #   kwag_['objective'] = Push_robot_14d(random_seed_, problem_id=kwag_['problem_id'])
            ...
            random_seed_ = sorted(generate_random_seed_coco())[i]
            kwag_["objective"] = NN_ML_Datasets(
                random_seed_, problem_id=kwag_["problem_id"]
            )
        else:
