...
class Problem(object):
    ...
    def __call__(self, x_unorder):
        ...
        if w == 0:
            ...
            R = max(
                np.sqrt(l ** 2 + (h + t) ** 2) / 2, np.sqrt(t ** 2 + (h + l) ** 2) / 2
            )
            costheta = l / (2 * R)
        if m == 0:
        ...
        pc = (
            ...
            * t
            * (b ** 3)
            * (np.sqrt(E * G)) ** (1 - (t * np.sqrt(E) / 4 * L * np.sqrt(G)))
            / (6 * (L ** 2))
        )
        tau1 = F / A
        tau2 = F * (L + 0.05 * l) * R / J
...
class Weld_Design(object):
    def __init__(self, random_seed=None, problem_id=None):
        ...
        self.problem = Problem(
            dimension=6, lower_bounds=lower_bounds, upper_bounds=upper_bounds
        )
        self.problem.dimension = 6
        self.num_discrete = 2
        self.num_continuous = 4
        print(
            f"num_discrete: {self.num_discrete}, num_continuous: {self.num_continuous}"
        )
        self.n_vertices = [2, 4]
        # for i in range(2):
        #     self.n_vertices.append(10) # m values
        self.n_vertices = np.array(self.n_vertices)
        self.suggested_init = self.generate_random_points(
            n_points=10, random_seed=random_seed
        )
        self.adjacency_mat = []
        self.fourier_freq = []
        self.fourier_basis = []
