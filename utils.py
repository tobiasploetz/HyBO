...
def displaying_and_logging(
    ...
    logfile = open(
        os.path.join(logfile_dir, str(eval_inputs.size(0)).zfill(4) + ".out"), "w"
    )
    for i in range(eval_inputs.size(0)):
        ...
        data_str = (
            "%3d-th : %+12.4f, mean : %+.4E, std : %.4E, var : %.4E, min : %+8.4f(%3d)"
            % (
                i + 1,
    ...
    logfile.close()
    if store_data:
        pklfilename = os.path.join(
            logfile_dir, str(eval_inputs.size(0)).zfill(4) + ".pkl"
        )
        torch.save(
            {
                "inputs": eval_inputs,
